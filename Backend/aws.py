...
#---
inst_id= fs.getvalue("inst_id")
#----
bucket_name= fs.getvalue("bucket_name")
region= fs.getvalue("region")
...
if choice == 1:
    ...
    output = sp.getoutput("sudo aws ec2 describe-instances --region {}".format(region))
    print("<span><br>#### Instance Description -{} ####</span><br>".format(region))
    print("<pre>{}</pre>".format(output))
    
elif choice == 8:
    output = sp.getoutput("sudo aws ec2 start-instances --instance-ids {} --region {}".format(inst_id,region))
    print("<span><br>#### Start Instance - {} ####</span><br>".format(vol_name))
    print("<pre>{}</pre>".format(output))    
    
elif choice == 9:
    output = sp.getoutput("sudo aws ec2 stop-instances --instance-ids {} --region {}".format(inst_id,region))
    print("<span><br>#### Stop Instance - {} ####</span><br>".format(vol_name))
    print("<pre>{}</pre>".format(output))   
    
elif choice == 10:
    output = sp.getoutput("sudo aws s3api create-bucket --bucket {} --region {}".format(bucket_name,region))
    print("<span><br>#### Create Bucket - {} ####</span><br>".format(bucket_name))
    print("<pre>{}</pre>".format(output))  
