function async(){
  ...
  && ((selectInputValue.includes("create")) || (selectInputValue.includes("execute")) || (selectInputValue.includes("launch")))) {
    ...
    xhr.open("GET", "http://3.16.214.58/cgi-bin/server.py?var="+desireInput, true);
    xhr.send();
    xhr.onload = function (){
...
function launchPod(){
    ...
    let xhr = new XMLHttpRequest();
    xhr.open("GET", `http://3.16.214.58/cgi-bin/server.py?var=kubectl run ${podName} --image=${imageName}`, true);
    xhr.send();
    xhr.onload = function (){
      let output = xhr.responseText;
      document.getElementById("baseContainer").innerHTML = output;
...
function listPods(){
    let xhr = new XMLHttpRequest();
    xhr.open("GET", `http://3.16.214.58/cgi-bin/server.py?var=kubectl get pods`, true);
    xhr.send();
    xhr.onload = function (){
      let output = xhr.responseText;
      document.getElementById("baseContainer").innerHTML = output;
...
function launchDeployment(){
    ...
    let xhr = new XMLHttpRequest();
    xhr.open("GET", `http://3.16.214.58/cgi-bin/server.py?var=kubectl create deployment ${deploymentName} --replicas=${replica} --image=${imageName}`, true);
    xhr.send();
    xhr.onload = function (){
      let output = xhr.responseText;
      document.getElementById("baseContainer").innerHTML = output;
...
function exposeService(){
    ...
    let xhr = new XMLHttpRequest();
    xhr.open("GET", `http://3.16.214.58/cgi-bin/server.py?var=kubectl expose deployment ${deploymentName} --port=${portNumber} --type=${lbName} `, true);
    xhr.send();
    xhr.onload = function (){
      let output = xhr.responseText;
      document.getElementById("baseContainer").innerHTML = output;
...
function removeService(){
    ...
    let xhr = new XMLHttpRequest();
    xhr.open("GET", `http://3.16.214.58/cgi-bin/server.py?var=kubectl delete ${serviceName} ${anything}`, true);
    xhr.send();
    xhr.onload = function (){
      let output = xhr.responseText;
      document.getElementById("baseContainer").innerHTML = output;
